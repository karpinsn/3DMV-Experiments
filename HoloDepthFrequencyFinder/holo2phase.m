...
stepHeight = (1.0 / fringeFrequency) - .00001; % .001 is just a buffer so we dont get rounding errors. At higher frequencies this causes errors
k = floor(holoimage(:,:,3) * 1.0 / stepHeight);

phaseMap = atan2(holoimage(:,:,1) - .5, holoimage(:,:,2) - .5) + (2 * pi * k);

% Add pi so that our phase starts at 0 instead of -pi
phaseMap = phaseMap + pi;
end
